...
How to format article content in this blog. Covers: dashes (global style rule), Mermaid diagrams, Wardley maps, sidenotes, and images. Links, citations, and reference lists are documented separately in the [URL and References Formatting Guide](url-and-references-formatting-guide.md).
...
Citations (inline footnote markers), the closing reference list, plain-list reference links, and the `resources` shortcode now live in their own document: see the [URL and References Formatting Guide](url-and-references-formatting-guide.md). It is the single source of truth for linking and citing sources, and applies to blog posts, ticker items, and project pages alike.
...
How to emit a `target="_blank"` link (and the required `rel="noopener"`) is documented in the [URL and References Formatting Guide](url-and-references-formatting-guide.md).
